# The SWIM Algorithm as Realized in swim-ex
...
Names match the source (`lib/swim_ex/`).

---

## Framework

Every node runs one `SwimEx.Protocol` GenServer. Two
recurring timers drive it, and incoming packets feed it:
...
Constants and their defaults (from `SwimEx.Protocol`):
...
startup from `System.system_time(:millisecond)`. Incoming
...
H1. Bump own incarnation to `System.system_time(:ms)` (so
    it outranks any rumor about the node).
